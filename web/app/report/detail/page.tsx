...
/**
 * 全データ画面 (`/report/detail`)。
 *
 * ★ 2026-08-12. 이 파일은 원래 `/report`였다. 그대로 옮겼다 — **한 줄도 지우지 않았다.**
 *   옮긴 이유: 심사위원이 이 화면에서 20초 안에 결론을 못 잡는다.
 *   전 실행 표 95칸 + 모델 고정 실험 + 用事별 결과 + findings 237건 + 원가 + 보안 +
 *   한계 5줄이 세로로 이어져 있고, **어느 것이 결론인지가 화면에 안 쓰여 있다.**
 *   옆에서 실행을 지켜본 사람에게는 읽히지만, 처음 보는 사람에게는 벽이다.
 *
 *   그래서 역할을 갈랐다 —
 *   · `/report`        : 결론만. 큰 숫자 3개 + 그래프 2개
 *   · **여기**          : 그 결론의 재료 전부. 「어디서 나온 숫자냐」에 답하는 자리
 *
 *   **지운 게 아니라 옮긴 것이다.** ⑥원가·⑦보안·한계 고지는 전부 여기 그대로 있다.
 *   심사에서 파고들면 이 URL 하나를 열면 된다.
 *
 * 원래 주석: website design/reports.png の並びをそのまま使う。
  ...
  return (
    <Page back={{ href: "/report", label: "← 結果のまとめへ" }}>
      <div className="mx-auto w-full max-w-6xl px-6 py-14">
        <p className="text-center text-xs font-medium tracking-widest text-fg-dim">
          ALL DATA
        </p>
