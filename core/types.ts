...
export type ActionKind =
  ...
  | "scroll_side"
  /**
   * 화면을 덮고 있는 것(안내 채팅·모달·쿠키 벽)을 닫는다.
   * **무언가가 실제로 화면 한가운데를 덮고 있을 때만** 후보에 오른다 (prompts.ts `allowedKinds`).
   *
   * 사람은 덮개가 뜨면 ✕를 누른다. 그런데 그 ✕는 링크도 버튼도 아닌 경우가 많아서
   * (`<div>`·`<svg>`에 스크립트로 붙인 것) 우리 추출기가 못 줍는다 — 즉 **에이전트에게만
   * 없는 수단**이었다. 그러면 「사이트가 어려워서 못 갔다」와 「우리가 덮개를 못 닫아서
   * 못 갔다」가 구별되지 않고, 이탈률이 우리 사정으로 높게 나온다.
   *
   * 실측(2026-08-15): 東京電力 톱을 375px 폭으로 보면 안내 채팅이 「引越しの手続きをする
   * （開始・停止)」 링크 위에 겹친다. 누르면 채팅이 열려 화면 전체를 덮고, 그 뒤 같은 링크를
   * 5번 더 눌러도 전부 채팅에 막혔다. 그 20수 중 닫기 수단은 목록에 한 번도 없었다.
   */
  | "close_overlay"
  | "back"
  | "find_in_page"
...
export type ObservationSnapshot = {
  ...
  /**
   * 화면 위에 얹힌 것이 있었는가. optional인 이유는 `scroll.x`와 같다 —
   * 2026-08-15 이전 트레이스에는 이 필드가 없다. 없으면 「덮개가 없었다」가 아니라
   * **「재지 않았다」**로 읽는다.
   *
   * `close_found`를 함께 남기는 이유: 덮개는 있는데 닫는 것을 못 찾은 경우가
   * **사이트에 대한 발견**이기 때문이다. 나중에 세려면 그때 남겨 두는 수밖에 없다.
   */
  overlay?: { covering: boolean; close_found: boolean; close_by?: string | null };
  /**
...
export type Finding = {
  ...
  step_n: number;
  /**
   * 어떤 종류의 걸림인가. `Signal["kind"]`와 같은 값이다 (`agent/src/signals.ts`).
   *
   * ★ 왜 옮겨 싣는가 (2026-08-14): 신호는 이미 종류를 알고 있는데 리포트로 넘어올 때
   *   버려지고 있었다. 그래서 화면에는 「크다/중간/작다」밖에 없었고, 그건 색이지 정보가 아니다.
   *   종류가 남아 있으면 「같은 페이지로 되돌아왔다」와 「못 읽는 말이 있었다」가
   *   **그림만 보고 구별된다.** 값 자체는 코드가 정한 것이라 모델이 건드릴 수 없다.
   *
   * ⚠️ 08-14 이전에 저장된 트레이스에는 이 필드가 없다. 읽는 쪽은 **없을 때를 반드시 견딘다.**
   *   문자열로 두는 이유는 `core/types.ts`가 아무것도 import 하지 않기 때문이다 —
   *   union을 여기에 한 벌 더 적으면 signals.ts와 조용히 어긋난다.
   */
  kind?: string;
  url: string;
  /** 무엇이 막았는가 */
